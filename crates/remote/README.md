...
The `remote` crate contains the implementation of the Auto Kanban hosted API.
...
AUTOKANBAN_REMOTE_JWT_SECRET=your_base64_encoded_secret
...
Generate `AUTOKANBAN_REMOTE_JWT_SECRET` once using `openssl rand -base64 48` and copy the value into `.env.remote`.
...
```bash
export AK_SHARED_API_BASE=http://localhost:3000
