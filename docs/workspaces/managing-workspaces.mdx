...
|----------|-----------------|
| **macOS** | `/var/folders/.../T/auto-kanban/worktrees` (system temp) |
| **Linux** | `/var/tmp/auto-kanban/worktrees` |
| **Windows** | `%TEMP%\auto-kanban\worktrees` |

You can configure a custom location in **Settings → General → Workspace Directory**. When set, worktrees are stored in `{your-path}/.auto-kanban-workspaces`.
...
<Tab title="macOS">
```bash
du -sh $TMPDIR/auto-kanban/worktrees
```
</Tab>
<Tab title="Linux">
```bash
du -sh /var/tmp/auto-kanban/worktrees
```
</Tab>
<Tab title="Windows">
```powershell
Get-ChildItem "$env:TEMP\auto-kanban\worktrees" -Recurse | Measure-Object -Property Length -Sum
```
</Tab>
</Tabs>
...
If you manually delete a worktree directory (e.g., using `rm -rf` in terminal), Auto Kanban will **automatically recreate it** the next time you open that workspace. The worktree is recreated from the branch's last commit.
...
Auto Kanban automatically cleans up "orphaned" worktrees on startup - these are worktree directories that no longer have a matching workspace in the database (e.g., if the app crashed during deletion).
