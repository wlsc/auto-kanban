---
title: "Auto Kanban MCP Server"
description: "Configure the Auto Kanban MCP server"
---

Auto Kanban exposes a local MCP (Model Context Protocol) server, allowing you to manage projects and tasks from external MCP clients like Claude Desktop, Raycast, or even coding agents running within Auto Kanban itself.

<Note>
This page covers connecting **external MCP clients** to Auto Kanban's MCP server. For configuring MCP servers **within** Auto Kanban for your coding agents, see the [MCP Server Configuration](/integrations/mcp-server-configuration) guide.
</Note>

<Info>
Auto Kanban's MCP server is **local-only** - it runs on your computer and can only be accessed by applications installed locally. It cannot be accessed via publicly accessible URLs.
</Info>
...
This works if you're adding the Auto Kanban MCP server to any [supported coding agent](/supported-coding-agents) **within** Auto Kanban.

1. In Auto Kanban Settings, navigate to the "MCP Servers" page
2. In the "Popular servers" section, click on the Auto Kanban card
3. Click the `Save Settings` button

<Frame>
<img src="/images/vk-mcp-server-config.jpeg" alt="MCP Servers configuration page showing how to add Auto Kanban MCP server" />
</Frame>
...
The Auto Kanban MCP server provides the following tools for managing projects, tasks, and task execution:
...
3. **Automatic Task Generation**: Your MCP client will use the Auto Kanban MCP server to automatically create structured tasks in your project
...
Your MCP client will use the `create_task` tool to break this down into individual tasks and add them to your Auto Kanban project automatically.
...
### Internal Coding Agents (Within Auto Kanban)

A powerful workflow involves using coding agents within Auto Kanban that are also connected to the Auto Kanban MCP server:
...
4. **Automatic Population**: The agent uses the MCP server to populate individual tasks directly in the Auto Kanban interface
...
Raycast is a popular MCP client that can connect to Auto Kanban's MCP server. Here's how to configure it:
...
<Frame>
<img src="/images/vk-raycast-mcp-part-1.png" alt="Raycast MCP configuration - adding Auto Kanban server" />
</Frame>

Configure the Auto Kanban MCP server in Raycast by adding the server details.
</Tab>
...
Once configured, you'll see the Auto Kanban MCP server listed and ready to use in Raycast.
</Tab>
</Tabs>
