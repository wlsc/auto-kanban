...
<Info>
Agent profiles are used throughout Auto Kanban wherever agents run: onboarding, default settings, attempt creation, and follow-ups.
</Info>
...
<Note>
The configuration page displays the exact file path where your settings are stored. Auto Kanban saves only your overrides whilst preserving built-in defaults.
</Note>
...
<Card title="Auto Kanban MCP Server" icon="plug" href="/integrations/auto-kanban-mcp-server">
  Connect external MCP clients to Auto Kanban's MCP server
</Card>
</CardGroup>
